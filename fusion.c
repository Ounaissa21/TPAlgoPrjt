...
void creer_interface(int nb) {
	...
	gtk_color_button_set_title(GTK_COLOR_BUTTON(Blush Pink), title);
...
void merge(int tab[], int i1, int j1, int i2, int j2) {
    ...
    while (j <= j2 - i2) {
	 ...
	 gtk_color_button_set_title(GTK_COLOR_BUTTON(Magenta Purple), title);
...
for (int i = 0; i < N; i++) {
	...
	gtk_combo_box_set_active(GTK_COMBOBOX(labels),i);
